      ...
      {sidebarLinks.map((item) => {
        ...
        const LinkComponent = (
          <Link
            key={item.label}
            href={item.route}
            className={`flex items-center justify-start gap-4 bg-transparent p-4 ${isActive ? "primary-gradient rounded-lg" : "text-dark300_light900"}`}
          >
            <Image src={item.imgURL} key={item.label} alt={item.label} width={20} height={20} />
